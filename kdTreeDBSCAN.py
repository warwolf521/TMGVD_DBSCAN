...
def dbscan(data, minPts, eps, kdTree):
    """
    ...
    - kdTree: KDTree de scipy para búsqueda de vecinos.
    ...
    def regionQueryKdtree(point_idx):
        return kdTree.query_ball_point(data[point_idx], eps)
        
    for pointIdx in range(n):
        if not visited[pointIdx]:
            visited[pointIdx] = True
            neighbors = regionQueryKdtree(pointIdx)
            if len(neighbors) < minPts:
                ...
                while i < len(neighbors):
                    ...
                    if not visited[neighborIdx]:
                        ...
                        newNeighbors = regionQueryKdtree(neighborIdx)
...
def findMaxCurvature(data, k=1, kdtree=None):
    """
    ...
    distances, _ = kdtree.query(data, k=k + 1)  # k+1 porque incluye el propio punto
    sortedDists = np.sort(distances[:, k])    # Ordenar por la distancia al k-ésimo vecino
...
def calculateMinPts(data, eps, beta=0.5, kdtree=None):
    """
    ...
    neighborsCount = [len(kdtree.query_ball_point(point, eps)) for point in data]
...
def adaptiveDBSCAN(data, k=None, beta=0.5):
    ...
    for i in range(1, k + 1):
        print("K:" + str(i))
        eps = findMaxCurvature(data, k=i, kdtree=kdtree)
        print("Optimal Eps:" + str(eps))
        minPts = calculateMinPts(data, eps, beta, kdtree)
        print("Optimal MinPts:" + str(minPts))
        labels = dbscan(data, minPts, eps, kdtree)
